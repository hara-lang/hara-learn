...
test("inherits protected Hara colour tokens instead of redefining them locally", async () => {
  ...
  assert.deepEqual(
    ...
    [],
    `Learn CSS must not redefine protected Hara tokens: ${protectedDeclarations.join(", ")}`
  );

  const lowered = styles.toLowerCase();
  for (const colour of retiredLearnPalette) {
    assert.ok(!lowered.includes(colour), `retired Learn palette colour ${colour} must not return`);
  }
});
...
test("publishes the favicon supplied by the pinned visual language", async () => {
  ...
  assert.equal(
    ...
    "Learn favicon must remain byte-identical to the pinned Hara brand asset"
  );
});
